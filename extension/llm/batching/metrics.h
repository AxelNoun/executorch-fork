...
struct ET_EXPERIMENTAL GenerationMetrics {
  ...
  double decode_tokens_per_sec() const {
    ...
    return itl_sum_us > 0
        ? 1e6 * static_cast<double>(itl_count) / itl_sum_us
...
struct ET_EXPERIMENTAL EngineMetrics {
  ...
  // Summed over steps, so dividing by `steps` gives the mean. Sequences, not
  // tokens: a prefill chunk is one sequence and many tokens.
  std::uint64_t decode_seqs_total = 0;
  std::uint64_t prefill_seqs_total = 0;
  ...
  // decode_seqs_total this says how much of the eligible work the scheduler
  ...
  //
  // There is no decode-tokens-per-second here on purpose. A step mixing both
  // kinds runs them in one forward pass over one weight read, so no share of
  // its latency belongs to either. Dividing these totals by the time of the
  // steps that held them would make packing prefill alongside decode -- which
  // raises total throughput -- look like a decode regression. What the run
  // actually delivered is model_input_tokens_per_sec() and the per-generation
  // inter-token latencies; both are exact and neither moves with the mix.
  ...
  // Step latency charged once to every decode sequence in the step. A latency
  // may be counted for several sequences because each of them really did wait
  ...
  std::int64_t decode_wait_sum_us = 0;
  ...
  // Mean decode sequences per step, and the mean that were eligible. Raw
  ...
  double mean_admitted_decode_seqs() const {
    return steps > 0 ? static_cast<double>(decode_seqs_total) / steps : 0.0;
  }

  double mean_ready_decode_seqs() const {
    return steps > 0 ? static_cast<double>(ready_total) / steps : 0.0;
  }

  // Every sequence in the step, both kinds. How full the forward pass was,
  // which is the batching question; the decode figures above are the
  // scheduling one.
  double mean_step_seqs() const {
    return steps > 0
        ? static_cast<double>(decode_seqs_total + prefill_seqs_total) / steps
        : 0.0;
  }
  ...
  double admitted_ratio() const {
    return ready_total > 0
        ? static_cast<double>(decode_seqs_total) / ready_total
        : 0.0;
  }
  ...
  double idle_fraction() const {
    ...
    return wall > 0.0
        ? 1.0 - static_cast<double>(step_latency_sum_us) / wall
        : 0.0;
  }
  ...
  double generated_tokens_per_sec() const {
    const double wall = wall_us();
    return wall > 0.0
        ? 1e6 * static_cast<double>(total_generated_tokens) / wall
        : 0.0;
  }
  ...
  // Mean wall time a decode sequence waited per step it took part in. The
  // engine-side counterpart to per-generation inter-token latency, and a
  // cross-check on it.
  double mean_decode_wait_us() const {
    return decode_seqs_total > 0
        ? static_cast<double>(decode_wait_sum_us) / decode_seqs_total
        : 0.0;
  }
...
ET_EXPERIMENTAL inline std::string format_report(const EngineMetrics& m) {
  std::ostringstream os;
  os << "engine\n"
     << "  elapsed     " << detail::fixed(m.wall_us() / 1e6, 2) << " s, "
     << m.steps << " steps";
  if (m.steps_failed > 0) {
    os << " (" << m.steps_failed << " failed)";
  }
  os << ", " << detail::fixed(100.0 * m.idle_fraction(), 1) << "% idle\n"
     << "  generations " << m.generations_completed << " of "
     << m.generations_started << ": " << m.finished_stop_token
     << " stop token, " << m.finished_token_limit << " token limit, "
     << m.finished_cancelled << " cancelled, " << m.finished_failed
     << " failed\n"
     << "  generated   " << m.total_generated_tokens << " tokens -> "
     << detail::fixed(m.generated_tokens_per_sec(), 1) << " tok/s\n"
     << "  model input " << m.model_input_tokens() << " tokens -> "
     << detail::fixed(m.model_input_tokens_per_sec(), 1) << " tok/s ("
     << m.decode_tokens_total << " decode + " << m.prefill_tokens_total
     << " prompt)\n"
     << "  admitted    " << detail::fixed(m.mean_admitted_decode_seqs(), 2)
     << " of " << detail::fixed(m.mean_ready_decode_seqs(), 2)
     << " ready decode seqs ("
     << detail::fixed(100.0 * m.admitted_ratio(), 1) << "%)\n"
     << "  step size   " << detail::fixed(m.mean_step_seqs(), 2) << " seqs, "
     << detail::fixed(m.mean_step_tokens(), 2) << " tokens\n"
     << "  step time   mean "
     << detail::ms(
            m.steps > 0
                ? m.step_latency_sum_us / static_cast<std::int64_t>(m.steps)
                : 0)
     << " ms, max " << detail::ms(m.step_latency_max_us)
     << " ms, per-seq decode wait "
     << detail::fixed(m.mean_decode_wait_us() / 1000.0, 2) << " ms\n"
     << "  ttft        mean " << detail::ms(m.mean_ttft_us()) << " ms, min "
     << detail::ms(m.min_ttft_us()) << " ms, max "
     << detail::ms(m.ttft_max_us) << " ms over " << m.ttft_count
     << " generations\n"
     << "  step kinds  " << m.steps_with_decode << " decode, "
     << m.steps_with_prefill << " prefill, " << m.mixed_steps() << " both\n";
  return os.str();
}
