...
def _backend_config() -> ExecutorchBackendConfig:
    # The Arm runtime expects the delegate payload inline rather than in its
    # own segment, which is what every other Arm AOT path asks for.
    return ExecutorchBackendConfig(extract_delegate_segments=False)


def _edge_compile_config() -> EdgeCompileConfig:
    # The CLI disables edge verification on every Arm path, quantized or not.
...
class _QuantBoundaryOpBackend(OpBackend):
    """Rewrites the QDQ ops left outside the delegate into cortex_m kernels.

    The Cortex-M operator backend in miniature: for a fully delegated model the
    boundary quantize/dequantize is all that is left for it to lower.
    ...
    def lower(
        ...
        return _transform(exported_program, ReplaceQuantNodesPass())
...
class ArmRecipeProvider(BackendRecipeProvider):
    ...
    def _ethos_u_compile_spec(
        ...
        if isinstance(extra_flags, str) or not all(
            isinstance(flag, str) for flag in extra_flags
        ):
            # A bare string would be iterated into one flag per character.
            raise ValueError(
    ...
    def _build_recipe(
        ...
        return ExportRecipe(
            ...
            lowering_recipe=LoweringRecipe(
                ...
                op_backends=(
                    [_QuantBoundaryOpBackend()] if replace_quant_nodes else None
                ),
            ),
            executorch_backend_config=_backend_config(),
        )
