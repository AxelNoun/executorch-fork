...
def _lower_and_verify(
    ...
    # The signature needs its own spec lists, not just its own object:
    # `create_constant_placeholder`, which the contract above points at, inserts
    # into `input_specs` before rebinding the signature.
    signature = exported_program.graph_signature
    private._graph_signature = ExportGraphSignature(
        list(signature.input_specs), list(signature.output_specs)
    )
